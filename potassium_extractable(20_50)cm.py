...
nc_file = r'C:\Users\hp\Desktop\project execution\geopandas\New thematic maps\soil_maps(20_50)cm\soil potassium extractable(20_50)cm\masked_potassium_2.nc'
ds = xr.open_dataset(nc_file)

# Assuming the dataset contains 'bulk_density', 'lat', and 'lon' variable
potassium_extractable_20_50 = ds['Potassium, extractable, predicted mean at 20-50 cm depth']  # You might need to adjust variable names based on your NetCDF file structure
lat = ds['lat']
lon = ds['lon']
...
#48.40245438, 180.27220154
values = potassium_extractable_final_20_50.values
...
contour = ax.contourf(lon, lat, combined_classes_k_20_50, cmap=cmap, levels=boundaries, norm=norm)
...
# Add colorbar
#cbar = plt.colorbar(contour, ax=ax, label='Soil Potassium Extractable Mean_20_50 Values (g/kg)', norm=norm, boundaries=boundaries, ticks=boundaries)
ax.set_title('Soil Potassium Extractable (20-50)cm')
...
#plt.title('Distribution of Soil Potassium Extractable Mean_20_50 Values (g/kg)')
#plt.xlabel('Soil Potassium Extractable Mean_20_50 Values (g/kg)')
