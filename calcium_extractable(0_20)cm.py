...
nc_file = r'C:\Users\hp\Desktop\project execution\geopandas\New thematic maps\soil_maps(0_20)cm\soil calcium extractable(0_20)cm\masked_calcium.nc'
ds = xr.open_dataset(nc_file)

# Assuming the dataset contains 'bulk_density', 'lat', and 'lon' variables
calcium_extractable = ds['Calcium, extractable, predicted mean at 0-20 cm depth']  # You might need to adjust variable names based on your NetCDF file structure
lat = ds['lat']
lon = ds['lon']
...
values = calcium_extractable_final.values
...
contour = ax.contourf(lon, lat, combined_classes_calcium, cmap=cmap, levels=boundaries, norm=norm)
...
# Add colorbar
#cbar = plt.colorbar(contour, ax=ax, label='Calcium Extractable Mean_0_20 Values (g/kg)', norm=norm, boundaries=boundaries, ticks=boundaries)
ax.set_title('Soil Calcium (0-20)cm')
...
#plt.title('Distribution of Calcium Extractable Mean_0_20 Values (g/kg)')
#plt.xlabel('Calcium Extractable Mean_0_20 Values (g/kg)')
