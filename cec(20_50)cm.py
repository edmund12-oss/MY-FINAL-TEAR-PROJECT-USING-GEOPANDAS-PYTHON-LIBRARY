...
nc_file = r'C:\Users\hp\Desktop\project execution\geopandas\New thematic maps\soil_maps(20_50)cm\soil cation exchange capacity(20_50)cm\masked_cec_2.nc'
ds = xr.open_dataset(nc_file)

# Assuming the dataset contains 'bulk_density', 'lat', and 'lon' variable
cec_20_50 = ds['Effective Cation Exchange Capacity, predicted mean at 20-50 cm depth']  # You might need to adjust variable names based on your NetCDF file structure
lat = ds['lat']
lon = ds['lon']
...
values = ds['Effective Cation Exchange Capacity, predicted mean at 20-50 cm depth'].values
...
contour = ax.contourf(lon, lat, combined_classes_cec_20_50, cmap=cmap, levels=boundaries, norm=norm)
...
# Add colorbar
#cbar = plt.colorbar(contour, ax=ax, label='Effective Cation Exchange Capacity Mean_20_50 Values (cmol+kg-1)', norm=norm, boundaries=boundaries, ticks=boundaries)
ax.set_title('Soil Effective Cation Exchange Capacity (20-50)cm')
...
#plt.title('Distribution of Effective C.E.C Mean_20_50 Values (cmol+kg-1)')
#plt.xlabel('Effective Cation Exchange Capacity Mean_20_50 Values (cmol+kg-1)')
