...
nc_file = r'C:\Users\hp\Desktop\project execution\geopandas\New thematic maps\soil_maps(0_20)cm\soil phosphorous extractable(0_20)cm\masked_phosphorus.nc'
ds = xr.open_dataset(nc_file)

# Assuming the dataset contains 'bulk_density', 'lat', and 'lon' variable
phosphorus_extractable = ds['Phosphorus, extractable, predicted mean at 0-20 cm depth']  # You might need to adjust variable names based on your NetCDF file structure
lat = ds['lat']
lon = ds['lon']
...
#3.95303249, 21.19794846
values = ds['Phosphorus, extractable, predicted mean at 0-20 cm depth'].values
...
contour = ax.contourf(lon, lat, combined_classes_phosphorus, cmap=cmap, levels=boundaries, norm=norm)
...
# Add colorbar
#cbar = plt.colorbar(contour, ax=ax, label='Soil Phosphorus Mean_0_20 Values (mg/kg)', norm=norm, boundaries=boundaries, ticks=boundaries)
ax.set_title('Soil Phosphorus (0-20)cm')
...
#plt.title('Distribution of Soil Phosphorus Mean_0_20 Values (mg/kg)')
#plt.xlabel('Soil Phosphorus Mean_0_20 Values (mg/kg)')
