...
nc_file = r'C:\Users\hp\Desktop\project execution\geopandas\New thematic maps\soil_maps(20_50)cm\soil sulphur extractable(20_50)cm\masked_sulphur_2.nc'
ds = xr.open_dataset(nc_file)

# Assuming the dataset contains 'bulk_density', 'lat', and 'lon' variable
sulphur_extractable_20_50 = ds['Sulphur, extractable, predicted mean at 20-50 cm depth']  # You might need to adjust variable names based on your NetCDF file structure
lat = ds['lat']
lon = ds['lon']
...
values = ds['Sulphur, extractable, predicted mean at 20-50 cm depth'].values
...
contour = ax.contourf(lon, lat, combined_classes_sulphur_20_50, cmap=cmap, levels=boundaries, norm=norm)
...
# Add colorbar
#cbar = plt.colorbar(contour, ax=ax, label='Soil Sulphur Extractable Mean_20_50 Values (mg/kg)', norm=norm, boundaries=boundaries, ticks=boundaries)
ax.set_title('Soil Sulphur Extractable (20-50)cm')
...
#plt.title('Distribution of Soil Sulphur Extractable Mean_20_50 Values (mg/kg)')
#plt.xlabel('Soil Sulphur Extractable Mean_20_50 Values (mg/kg)')
