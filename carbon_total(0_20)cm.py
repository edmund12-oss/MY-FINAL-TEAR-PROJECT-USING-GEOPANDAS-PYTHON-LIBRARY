...
nc_file = r'C:\Users\hp\Desktop\project execution\geopandas\New thematic maps\soil_maps(0_20)cm\soil carbon total(0_20)cm\masked_carbon_total.nc'
ds = xr.open_dataset(nc_file)

# Assuming the dataset contains 'bulk_density', 'lat', and 'lon' variables
carbon_total = ds['mean_0_20']  # You might need to adjust variable names based on your NetCDF file structure
#carbon_total_2d = carbon_total.isel(time=0)
lat = ds['lat']
lon = ds['lon']

#converting from g/kg to percentage 
carbon_total_percentage = carbon_total / 10
...
values = carbon_total_percentage.values
...
contour = ax.contourf(lon, lat, combined_classes_ct, cmap=cmap, levels=boundaries, norm=norm)
...
# Add colorbar
#cbar = plt.colorbar(contour, ax=ax, label='Soil Carbon Total Mean_0_20 Values (%)', norm=norm, boundaries=boundaries, ticks=boundaries)
ax.set_title('Soil Carbon Total (0-20)cm')
...
#plt.title('Distribution of Soil Carbon Total Mean_0_20 Values (%)')
#plt.xlabel('Soil Carbon Total Mean_0_20 Values (%)')
