...
nc_file = r'E:\project execution\geopandas\New thematic maps\windspeed map\wind_speed.nc'
ds = xr.open_dataset(nc_file)

# Assuming the dataset contains 'bulk_density', 'lat', and 'lon' variables
wind_speed = ds['__xarray_dataarray_variable__']# You might need to adjust variable names based on your NetCDF file structure
#wind_speed_2d = wind_speed.isel(time=0)
lat = ds['lat']
lon = ds['lon']

#1.5345645, 2.84886098 
...
boundaries = [1.5, 1.6, 1.7, 1.8, 1.9, 2.0, 2.1, 2.2, 2.3, 2.4, 2.5, 2.6, 2.7, 2.8, 2.9]  # Adjust these boundaries as needed
...
fig, ax = plt.subplots(figsize=(12, 8), alpha=0.7)
contour = ax.contourf(lon, lat, wind_speed, cmap=cmap, levels=boundaries, norm=norm)
...
# Add colorbar
cbar = plt.colorbar(contour, ax=ax, label='Windspeed Values (m/s)', norm=norm, boundaries=boundaries, ticks=boundaries)
ax.set_title('Windspeed Thematic Map')
...
plt.title('Distribution of Windspeed Values (m/s)')
plt.xlabel('Windspeed Values (m/s)')
...
#slope_path = r'E:\project execution\geopandas\New thematic maps\windspeed map\ws.tif'
#with rasterio.open(slope_path, 'w', **meta) as dst:
    #dst.write(wind_speed_2d.astype(rasterio.float32), 1)
