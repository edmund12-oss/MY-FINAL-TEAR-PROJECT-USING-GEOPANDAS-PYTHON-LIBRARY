...
nc_file = r'C:\Users\hp\Desktop\project execution\geopandas\New thematic maps\soil_maps(20_50)cm\soil zinc extractable(20_50)cm\masked_zinc_2.nc'
ds = xr.open_dataset(nc_file)

# Assuming the dataset contains 'bulk_density', 'lat', and 'lon' variable
zinc_extractable_20_50 = ds['Zinc, extractable, predicted mean at 20-50 cm depth']  # You might need to adjust variable names based on your NetCDF file structure
lat = ds['lat']
lon = ds['lon']
...
#1.45960307, 6.38905621
values = ds['Zinc, extractable, predicted mean at 20-50 cm depth'].values
...
contour = ax.contourf(lon, lat, combined_classes_zinc_20_50, cmap=cmap, levels=boundaries, norm=norm)
...
# Add colorbar
#cbar = plt.colorbar(contour, ax=ax, label='Soil Zinc Extractable Mean_20_50 Values (mg/kg)', norm=norm, boundaries=boundaries, ticks=boundaries)
ax.set_title('Soil Zinc Extractable (20-50)cm')
...
#plt.title('Distribution of Soil Zinc Extractable Mean_20_50 Values (mg/kg)')
#plt.xlabel('Soil Zinc Extractable Mean_20_50 Values (mg/kg)')
