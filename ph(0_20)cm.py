...
nc_file = r'C:\Users\hp\Desktop\project execution\geopandas\New thematic maps\soil_maps(0_20)cm\soil ph(0_20)cm\masked_ph.nc'
ds = xr.open_dataset(nc_file)

# Assuming the dataset contains 'bulk_density', 'lat', and 'lon' variable
ph = ds['pH, predicted mean at 0-20 cm depth']  # You might need to adjust variable names based on your NetCDF file structure
lat = ds['lat']
lon = ds['lon']
...
values = ds['pH, predicted mean at 0-20 cm depth'].values
...
contour = ax.contourf(lon, lat, combined_classes_ph, cmap=cmap, levels=boundaries, norm=norm)
...
# Add colorbar
#cbar = plt.colorbar(contour, ax=ax, label='Soil pH Mean_0_20 Values', norm=norm, boundaries=boundaries, ticks=boundaries)
ax.set_title('Soil pH (0-20)cm')
...
#plt.title('Distribution of Soil pH Mean_0_20 Values')
#plt.xlabel('Soil pH Mean_0_20 Values')
